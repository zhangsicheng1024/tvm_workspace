...
for i in range(num):
    ...
    metrics_path = os.path.join(ncu_dir, 'dwconv_' + str(i) + '_metrics')
    latency_path = os.path.join(ncu_dir, 'dwconv_' + str(i) + '_latency')
    # f_out.write('ncu --section SpeedOfLight --metrics \"' + query_list + query_list_mem_throughput + '\" --log-file dwconv_fds_300_ncu/dwconv_'+str(i)+" python dwconv_rerun.py --config1 ./dwconv_fds_300_json/dwconv_"+str(i)+".json\n")
    f.write('nvprof --metrics \"' + query_list + query_list_mem_throughput + '\" --log-file ' + metrics_path + ' python dwconv_rerun.py --config1 ' + json_path + '\n')
    f.write('nvprof --log-file ' + latency_path + ' python dwconv_rerun.py --config1 ' + json_path + '\n')
