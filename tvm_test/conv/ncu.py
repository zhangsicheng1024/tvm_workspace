...
for i in range(num):
    json_path = os.path.join(json_dir, 'conv_' + str(i) + '.json')
    metrics_path = os.path.join(ncu_dir, 'conv_' + str(i) + '_metrics')
    latency_path = os.path.join(ncu_dir, 'conv_' + str(i) + '_latency')
    # f_out.write('ncu --section SpeedOfLight --metrics \"' + query_list + query_list_mem_throughput + '\" --log-file conv_fds_300_ncu/conv_'+str(i)+" python conv_rerun.py --config1 ./conv_fds_300_json/conv_"+str(i)+".json\n")
    f.write('nvprof --metrics \"' + query_list + query_list_mem_throughput + '\" --log-file ' + metrics_path + ' python conv_rerun.py --config1 ' + json_path + '\n')
    f.write('nvprof --log-file ' + latency_path + ' python conv_rerun.py --config1 ' + json_path + '\n')
