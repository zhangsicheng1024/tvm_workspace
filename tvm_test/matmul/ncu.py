...
for i in range(num):
    ...
    metrics_path = os.path.join(ncu_dir, 'matmul_' + str(i) + '_metrics')
    latency_path = os.path.join(ncu_dir, 'matmul_' + str(i) + '_latency')
    # f_out.write('ncu --section SpeedOfLight --metrics \"' + query_list + query_list_mem_throughput + '\" --log-file matmul_fds_300_ncu/matmul_'+str(i)+" python matmul_rerun.py --config1 ./matmul_fds_300_json/matmul_"+str(i)+".json\n")
    f.write('nvprof --metrics \"' + query_list + query_list_mem_throughput + '\" --log-file ' + metrics_path + ' python matmul_rerun.py --config1 ' + json_path + '\n')
    f.write('nvprof --log-file ' + latency_path + ' python matmul_rerun.py --config1 ' + json_path + '\n')
