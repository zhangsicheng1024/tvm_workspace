...
extern "C" __global__ void __launch_bounds__(128) mymatmul_kernel0(float* __restrict__ data, float* __restrict__ kernel, float* __restrict__ T_matmul_NN) {
  ...
  for (int k_outer_outer = 0; k_outer_outer < 128; ++k_outer_outer) {
    __syncthreads();
    data_shared[((int)threadIdx.x)] = data[(((((((int)blockIdx.x) >> 4) * 16384) + ((((int)threadIdx.x) >> 3) * 1024)) + (k_outer_outer * 8)) + (((int)threadIdx.x) & 7))];
    for (int ax0_ax1_fused_outer_outer = 0; ax0_ax1_fused_outer_outer < 4; ++ax0_ax1_fused_outer_outer) {
      *(float4*)(kernel_shared + ((ax0_ax1_fused_outer_outer * 512) + (((int)threadIdx.x) * 4))) = *(float4*)(kernel + (((((k_outer_outer * 32768) + (ax0_ax1_fused_outer_outer * 8192)) + ((((int)threadIdx.x) >> 6) * 4096)) + ((((int)blockIdx.x) & 15) * 256)) + ((((int)threadIdx.x) & 63) * 4)));
    }
    __syncthreads();
      ...
      for (int i_c_outer_inner = 0; i_c_outer_inner < 4; ++i_c_outer_inner) {
        for (int k_inner = 0; k_inner < 2; ++k_inner) {
          for (int i_c_inner = 0; i_c_inner < 4; ++i_c_inner) {
            T_matmul_NN_local[((i_c_outer_inner * 4) + i_c_inner)] = (T_matmul_NN_local[((i_c_outer_inner * 4) + i_c_inner)] + (data_shared[((((i_c_outer_inner * 32) + (i_c_inner * 8)) + (k_outer_inner * 2)) + k_inner)] * kernel_shared[(((k_outer_inner * 512) + (k_inner * 256)) + ((int)threadIdx.x))]));
            T_matmul_NN_local[(((i_c_outer_inner * 4) + i_c_inner) + 16)] = (T_matmul_NN_local[(((i_c_outer_inner * 4) + i_c_inner) + 16)] + (data_shared[((((i_c_outer_inner * 32) + (i_c_inner * 8)) + (k_outer_inner * 2)) + k_inner)] * kernel_shared[((((k_outer_inner * 512) + (k_inner * 256)) + ((int)threadIdx.x)) + 128)]));
          }
        }
      }
    }
