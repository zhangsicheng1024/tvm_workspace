...
extern "C" __global__ void __launch_bounds__(64) mymatmul_kernel0(float* __restrict__ data, float* __restrict__ kernel, float* __restrict__ T_matmul_NN) {
  ...
  for (int i_c_outer_inner_init = 0; i_c_outer_inner_init < 2; ++i_c_outer_inner_init) {
    for (int j_c_outer_inner_init = 0; j_c_outer_inner_init < 4; ++j_c_outer_inner_init) {
      for (int i_c_inner_init = 0; i_c_inner_init < 16; ++i_c_inner_init) {
        T_matmul_NN_local[(((i_c_outer_inner_init * 64) + (i_c_inner_init * 4)) + j_c_outer_inner_init)] = 0.000000e+00f;
      }
    }
  }
  for (int k_outer_outer = 0; k_outer_outer < 128; ++k_outer_outer) {
    ...
    for (int ax0_ax1_fused_outer_outer = 0; ax0_ax1_fused_outer_outer < 4; ++ax0_ax1_fused_outer_outer) {
      data_shared[((ax0_ax1_fused_outer_outer * 64) + ((int)threadIdx.x))] = data[((((((((int)blockIdx.x) >> 4) * 32768) + (ax0_ax1_fused_outer_outer * 8192)) + ((((int)threadIdx.x) >> 3) * 1024)) + (k_outer_outer * 8)) + (((int)threadIdx.x) & 7))];
    }
    for (int ax0_ax1_fused_outer_outer_1 = 0; ax0_ax1_fused_outer_outer_1 < 32; ++ax0_ax1_fused_outer_outer_1) {
      kernel_shared[((ax0_ax1_fused_outer_outer_1 * 64) + ((int)threadIdx.x))] = kernel[(((((k_outer_outer * 32768) + ((ax0_ax1_fused_outer_outer_1 >> 2) * 4096)) + ((((int)blockIdx.x) & 15) * 256)) + ((ax0_ax1_fused_outer_outer_1 & 3) * 64)) + ((int)threadIdx.x))];
    }
    __syncthreads();
      ...
      for (int i_c_outer_inner = 0; i_c_outer_inner < 2; ++i_c_outer_inner) {
        for (int j_c_outer_inner = 0; j_c_outer_inner < 4; ++j_c_outer_inner) {
          for (int i_c_inner = 0; i_c_inner < 16; ++i_c_inner) {
            T_matmul_NN_local[(((i_c_outer_inner * 64) + (i_c_inner * 4)) + j_c_outer_inner)] = (T_matmul_NN_local[(((i_c_outer_inner * 64) + (i_c_inner * 4)) + j_c_outer_inner)] + (data_shared[(((i_c_outer_inner * 128) + (i_c_inner * 8)) + k_outer_inner)] * kernel_shared[(((k_outer_inner * 256) + (((int)threadIdx.x) * 4)) + j_c_outer_inner)]));
          }
        }
      }
    }
  ...
  for (int i_inner = 0; i_inner < 32; ++i_inner) {
    for (int j_inner = 0; j_inner < 4; ++j_inner) {
      T_matmul_NN[((((((((int)blockIdx.x) >> 4) * 131072) + (i_inner * 4096)) + ((((int)blockIdx.x) & 15) * 256)) + (((int)threadIdx.x) * 4)) + j_inner)] = T_matmul_NN_local[((i_inner * 4) + j_inner)];
    }
  }
}
