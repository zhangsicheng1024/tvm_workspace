...
extern "C" __global__ void __launch_bounds__(256) mymatmul_kernel0(float* __restrict__ data, float* __restrict__ kernel, float* __restrict__ T_matmul_NN) {
  ...
  for (int i_c_outer_inner_init = 0; i_c_outer_inner_init < 2; ++i_c_outer_inner_init) {
    for (int j_c_outer_inner_init = 0; j_c_outer_inner_init < 8; ++j_c_outer_inner_init) {
      for (int i_c_inner_init = 0; i_c_inner_init < 4; ++i_c_inner_init) {
        T_matmul_NN_local[(((i_c_outer_inner_init * 32) + (i_c_inner_init * 8)) + j_c_outer_inner_init)] = 0.000000e+00f;
      }
    }
  }
  for (int k_outer_outer = 0; k_outer_outer < 256; ++k_outer_outer) {
    ...
    if (((int)threadIdx.x) < 64) {
      data_shared[((int)threadIdx.x)] = data[(((((((int)blockIdx.x) >> 2) * 16384) + ((((int)threadIdx.x) >> 2) * 1024)) + (k_outer_outer * 4)) + (((int)threadIdx.x) & 3))];
    }
    for (int ax0_ax1_fused_outer_outer = 0; ax0_ax1_fused_outer_outer < 16; ++ax0_ax1_fused_outer_outer) {
      kernel_shared[((ax0_ax1_fused_outer_outer * 256) + ((int)threadIdx.x))] = kernel[(((((k_outer_outer * 16384) + ((ax0_ax1_fused_outer_outer >> 2) * 4096)) + ((((int)blockIdx.x) & 3) * 1024)) + ((ax0_ax1_fused_outer_outer & 3) * 256)) + ((int)threadIdx.x))];
    }
    __syncthreads();
        ...
        for (int j_c_outer_inner = 0; j_c_outer_inner < 8; ++j_c_outer_inner) {
          for (int k_inner = 0; k_inner < 2; ++k_inner) {
            for (int i_c_inner = 0; i_c_inner < 4; ++i_c_inner) {
              T_matmul_NN_local[(((i_c_outer_inner * 32) + (i_c_inner * 8)) + j_c_outer_inner)] = (T_matmul_NN_local[(((i_c_outer_inner * 32) + (i_c_inner * 8)) + j_c_outer_inner)] + (data_shared[((((((((int)threadIdx.x) >> 7) * 32) + (i_c_outer_inner * 16)) + (i_c_inner * 4)) + (k_outer_inner * 2)) + k_inner)] * kernel_shared[((((k_outer_inner * 2048) + (k_inner * 1024)) + ((((int)threadIdx.x) & 127) * 8)) + j_c_outer_inner)]));
            }
          }
        }
      }
  ...
  for (int i_inner = 0; i_inner < 8; ++i_inner) {
    for (int j_inner = 0; j_inner < 8; ++j_inner) {
      T_matmul_NN[(((((((((int)blockIdx.x) >> 2) * 65536) + ((((int)threadIdx.x) >> 7) * 32768)) + (i_inner * 4096)) + ((((int)blockIdx.x) & 3) * 1024)) + ((((int)threadIdx.x) & 127) * 8)) + j_inner)] = T_matmul_NN_local[((i_inner * 8) + j_inner)];
    }
  }
}
