...
extern "C" __global__ void __launch_bounds__(16) mymatmul_kernel0(float* __restrict__ data, float* __restrict__ kernel, float* __restrict__ T_matmul_NN) {
  ...
  for (int k_outer_outer = 0; k_outer_outer < 512; ++k_outer_outer) {
    ...
    for (int ax0_ax1_fused_outer_outer = 0; ax0_ax1_fused_outer_outer < 16; ++ax0_ax1_fused_outer_outer) {
      data_shared[((ax0_ax1_fused_outer_outer * 16) + ((int)threadIdx.x))] = data[((((((((int)blockIdx.x) >> 5) * 131072) + (ax0_ax1_fused_outer_outer * 8192)) + ((((int)threadIdx.x) >> 1) * 1024)) + (k_outer_outer * 2)) + (((int)threadIdx.x) & 1))];
    }
    for (int ax0_ax1_fused_outer_outer_1 = 0; ax0_ax1_fused_outer_outer_1 < 16; ++ax0_ax1_fused_outer_outer_1) {
      kernel_shared[((ax0_ax1_fused_outer_outer_1 * 16) + ((int)threadIdx.x))] = kernel[(((((k_outer_outer * 8192) + ((ax0_ax1_fused_outer_outer_1 >> 3) * 4096)) + ((((int)blockIdx.x) & 31) * 128)) + ((ax0_ax1_fused_outer_outer_1 & 7) * 16)) + ((int)threadIdx.x))];
    }
    __syncthreads();
    for (int k_outer_inner = 0; k_outer_inner < 2; ++k_outer_inner) {
      for (int i_c_outer_inner = 0; i_c_outer_inner < 4; ++i_c_outer_inner) {
        for (int j_c_outer_inner = 0; j_c_outer_inner < 128; ++j_c_outer_inner) {
          T_matmul_NN_local[((i_c_outer_inner * 128) + j_c_outer_inner)] = (T_matmul_NN_local[((i_c_outer_inner * 128) + j_c_outer_inner)] + (data_shared[(((((int)threadIdx.x) * 8) + (i_c_outer_inner * 2)) + k_outer_inner)] * kernel_shared[((k_outer_inner * 128) + j_c_outer_inner)]));
          T_matmul_NN_local[(((i_c_outer_inner * 128) + j_c_outer_inner) + 512)] = (T_matmul_NN_local[(((i_c_outer_inner * 128) + j_c_outer_inner) + 512)] + (data_shared[((((((int)threadIdx.x) * 8) + (i_c_outer_inner * 2)) + k_outer_inner) + 128)] * kernel_shared[((k_outer_inner * 128) + j_c_outer_inner)]));
        }
      }
    }
  }
