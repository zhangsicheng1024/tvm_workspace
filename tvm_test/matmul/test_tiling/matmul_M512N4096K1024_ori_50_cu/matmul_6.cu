    ...
    for (int j_c_outer_inner = 0; j_c_outer_inner < 2; ++j_c_outer_inner) {
      for (int k_inner = 0; k_inner < 2; ++k_inner) {
        for (int i_c_inner = 0; i_c_inner < 4; ++i_c_inner) {
          T_matmul_NN_local[((i_c_inner * 2) + j_c_outer_inner)] = (T_matmul_NN_local[((i_c_inner * 2) + j_c_outer_inner)] + (data_shared[((((((int)threadIdx.x) >> 6) * 8) + (i_c_inner * 2)) + k_inner)] * kernel_shared[(((k_inner * 512) + ((((int)threadIdx.x) & 63) * 2)) + j_c_outer_inner)]));
