...
extern "C" __global__ void __launch_bounds__(128) mymatmul_kernel0(float* __restrict__ data, float* __restrict__ kernel, float* __restrict__ T_matmul_NN) {
  ...
  for (int k_outer_outer = 0; k_outer_outer < 128; ++k_outer_outer) {
    ...
    for (int ax0_ax1_fused_outer_outer = 0; ax0_ax1_fused_outer_outer < 16; ++ax0_ax1_fused_outer_outer) {
      kernel_shared[((ax0_ax1_fused_outer_outer * 128) + ((int)threadIdx.x))] = kernel[(((((k_outer_outer * 32768) + ((ax0_ax1_fused_outer_outer >> 1) * 4096)) + ((((int)blockIdx.x) & 15) * 256)) + ((ax0_ax1_fused_outer_outer & 1) * 128)) + ((int)threadIdx.x))];
    }
    __syncthreads();
    for (int k_outer_inner = 0; k_outer_inner < 8; ++k_outer_inner) {
      for (int i_c_outer_inner = 0; i_c_outer_inner < 4; ++i_c_outer_inner) {
        for (int i_c_inner = 0; i_c_inner < 4; ++i_c_inner) {
          T_matmul_NN_local[((i_c_outer_inner * 4) + i_c_inner)] = (T_matmul_NN_local[((i_c_outer_inner * 4) + i_c_inner)] + (data_shared[(((i_c_outer_inner * 32) + (i_c_inner * 8)) + k_outer_inner)] * kernel_shared[((k_outer_inner * 256) + ((int)threadIdx.x))]));
          T_matmul_NN_local[(((i_c_outer_inner * 4) + i_c_inner) + 16)] = (T_matmul_NN_local[(((i_c_outer_inner * 4) + i_c_inner) + 16)] + (data_shared[(((i_c_outer_inner * 32) + (i_c_inner * 8)) + k_outer_inner)] * kernel_shared[(((k_outer_inner * 256) + ((int)threadIdx.x)) + 128)]));
        }
      }
    }
  }
