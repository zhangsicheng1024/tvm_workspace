  ...
  for (int i_c_outer_inner_init = 0; i_c_outer_inner_init < 2; ++i_c_outer_inner_init) {
    for (int i_c_inner_init = 0; i_c_inner_init < 8; ++i_c_inner_init) {
      for (int j_c_inner_init = 0; j_c_inner_init < 16; ++j_c_inner_init) {
        T_matmul_NN_local[(((i_c_outer_inner_init * 128) + (i_c_inner_init * 16)) + j_c_inner_init)] = 0.000000e+00f;
        T_matmul_NN_local[((((i_c_outer_inner_init * 128) + (i_c_inner_init * 16)) + j_c_inner_init) + 256)] = 0.000000e+00f;
      }
    }
  }
        ...
        for (int k_inner = 0; k_inner < 8; ++k_inner) {
          for (int i_c_inner = 0; i_c_inner < 8; ++i_c_inner) {
            for (int j_c_inner = 0; j_c_inner < 16; ++j_c_inner) {
              T_matmul_NN_local[(((i_c_outer_inner * 128) + (i_c_inner * 16)) + j_c_inner)] = (T_matmul_NN_local[(((i_c_outer_inner * 128) + (i_c_inner * 16)) + j_c_inner)] + (data_shared[((((((((int)threadIdx.x) >> 4) * 256) + (i_c_outer_inner * 128)) + (i_c_inner * 16)) + (k_outer_inner * 8)) + k_inner)] * kernel_shared[((((k_outer_inner * 4096) + (k_inner * 512)) + ((((int)threadIdx.x) & 15) * 16)) + j_c_inner)]));
              T_matmul_NN_local[((((i_c_outer_inner * 128) + (i_c_inner * 16)) + j_c_inner) + 256)] = (T_matmul_NN_local[((((i_c_outer_inner * 128) + (i_c_inner * 16)) + j_c_inner) + 256)] + (data_shared[((((((((int)threadIdx.x) >> 4) * 256) + (i_c_outer_inner * 128)) + (i_c_inner * 16)) + (k_outer_inner * 8)) + k_inner)] * kernel_shared[(((((k_outer_inner * 4096) + (k_inner * 512)) + ((((int)threadIdx.x) & 15) * 16)) + j_c_inner) + 256)]));
            }
          }
        }
      }
