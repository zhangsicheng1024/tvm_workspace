    ...
    for (int k_outer_inner = 0; k_outer_inner < 4; ++k_outer_inner) {
      for (int k_inner = 0; k_inner < 2; ++k_inner) {
        for (int i_c_inner = 0; i_c_inner < 16; ++i_c_inner) {
          T_matmul_NN_local[i_c_inner] = (T_matmul_NN_local[i_c_inner] + (data_shared[(((i_c_inner * 8) + (k_outer_inner * 2)) + k_inner)] * kernel_shared[(((k_outer_inner * 512) + (k_inner * 256)) + ((int)threadIdx.x))]));
          T_matmul_NN_local[(i_c_inner + 16)] = (T_matmul_NN_local[(i_c_inner + 16)] + (data_shared[(((i_c_inner * 8) + (k_outer_inner * 2)) + k_inner)] * kernel_shared[((((k_outer_inner * 512) + (k_inner * 256)) + ((int)threadIdx.x)) + 128)]));
        }
      }
    }
  }
