...
extern "C" __global__ void __launch_bounds__(32) mymatmul_kernel0(float* __restrict__ data, float* __restrict__ kernel, float* __restrict__ T_matmul_NN) {
  ...
  for (int k_outer_outer = 0; k_outer_outer < 32; ++k_outer_outer) {
    ...
    for (int ax0_ax1_fused_outer_outer = 0; ax0_ax1_fused_outer_outer < 8; ++ax0_ax1_fused_outer_outer) {
      *(float4*)(data_shared + ((ax0_ax1_fused_outer_outer * 128) + (((int)threadIdx.x) * 4))) = *(float4*)(data + ((((((((int)blockIdx.x) >> 5) * 32768) + (ax0_ax1_fused_outer_outer * 4096)) + ((((int)threadIdx.x) >> 3) * 1024)) + (k_outer_outer * 32)) + ((((int)threadIdx.x) & 7) * 4)));
    }
    for (int ax0_ax1_fused_outer_outer_1 = 0; ax0_ax1_fused_outer_outer_1 < 32; ++ax0_ax1_fused_outer_outer_1) {
      *(float4*)(kernel_shared + ((ax0_ax1_fused_outer_outer_1 * 128) + (((int)threadIdx.x) * 4))) = *(float4*)(kernel + ((((k_outer_outer * 131072) + (ax0_ax1_fused_outer_outer_1 * 4096)) + ((((int)blockIdx.x) & 31) * 128)) + (((int)threadIdx.x) * 4)));
    }
    __syncthreads();
        ...
        for (int k_inner = 0; k_inner < 16; ++k_inner) {
          for (int i_c_inner = 0; i_c_inner < 4; ++i_c_inner) {
            for (int j_c_inner = 0; j_c_inner < 4; ++j_c_inner) {
              T_matmul_NN_local[(((i_c_outer_inner * 16) + (i_c_inner * 4)) + j_c_inner)] = (T_matmul_NN_local[(((i_c_outer_inner * 16) + (i_c_inner * 4)) + j_c_inner)] + (data_shared[((((i_c_outer_inner * 128) + (i_c_inner * 32)) + (k_outer_inner * 16)) + k_inner)] * kernel_shared[((((k_outer_inner * 2048) + (k_inner * 128)) + (((int)threadIdx.x) * 4)) + j_c_inner)]));
            }
          }
        }
      }
  ...
  for (int i_inner = 0; i_inner < 32; ++i_inner) {
    for (int j_inner = 0; j_inner < 4; ++j_inner) {
      T_matmul_NN[((((((((int)blockIdx.x) >> 5) * 131072) + (i_inner * 4096)) + ((((int)blockIdx.x) & 31) * 128)) + (((int)threadIdx.x) * 4)) + j_inner)] = T_matmul_NN_local[((i_inner * 4) + j_inner)];
    }
  }
}
