...
extern "C" __global__ void __launch_bounds__(64) mymatmul_kernel0(float* __restrict__ data, float* __restrict__ kernel, float* __restrict__ T_matmul_NN) {
    ...
    for (int j_c_outer_inner_init = 0; j_c_outer_inner_init < 4; ++j_c_outer_inner_init) {
      for (int i_c_inner_init = 0; i_c_inner_init < 4; ++i_c_inner_init) {
        for (int j_c_inner_init = 0; j_c_inner_init < 4; ++j_c_inner_init) {
          ...
          T_matmul_NN_local[(((((i_c_outer_inner_init * 64) + (i_c_inner_init * 16)) + (j_c_outer_inner_init * 4)) + j_c_inner_init) + 2048)] = 0.000000e+00f;
        }
      }
    }
  }
  for (int k_outer_outer = 0; k_outer_outer < 256; ++k_outer_outer) {
    ...
    for (int ax0_ax1_fused_outer_outer = 0; ax0_ax1_fused_outer_outer < 8; ++ax0_ax1_fused_outer_outer) {
      data_shared[((ax0_ax1_fused_outer_outer * 64) + ((int)threadIdx.x))] = data[((((((((int)blockIdx.x) >> 1) * 131072) + (ax0_ax1_fused_outer_outer * 16384)) + ((((int)threadIdx.x) >> 2) * 1024)) + (k_outer_outer * 4)) + (((int)threadIdx.x) & 3))];
    }
        ...
        for (int k_inner = 0; k_inner < 4; ++k_inner) {
          for (int i_c_inner = 0; i_c_inner < 4; ++i_c_inner) {
            for (int j_c_inner = 0; j_c_inner < 4; ++j_c_inner) {
              T_matmul_NN_local[((((i_c_outer_inner * 64) + (i_c_inner * 16)) + (j_c_outer_inner * 4)) + j_c_inner)] = (T_matmul_NN_local[((((i_c_outer_inner * 64) + (i_c_inner * 16)) + (j_c_outer_inner * 4)) + j_c_inner)] + (data_shared[(((i_c_outer_inner * 16) + (i_c_inner * 4)) + k_inner)] * kernel_shared[((((k_inner * 2048) + (((int)threadIdx.x) * 16)) + (j_c_outer_inner * 4)) + j_c_inner)]));
              T_matmul_NN_local[(((((i_c_outer_inner * 64) + (i_c_inner * 16)) + (j_c_outer_inner * 4)) + j_c_inner) + 2048)] = (T_matmul_NN_local[(((((i_c_outer_inner * 64) + (i_c_inner * 16)) + (j_c_outer_inner * 4)) + j_c_inner) + 2048)] + (data_shared[(((i_c_outer_inner * 16) + (i_c_inner * 4)) + k_inner)] * kernel_shared[(((((k_inner * 2048) + (((int)threadIdx.x) * 16)) + (j_c_outer_inner * 4)) + j_c_inner) + 1024)]));
            }
          }
        }
      }
  ...
  for (int i_inner = 0; i_inner < 128; ++i_inner) {
    for (int j_inner = 0; j_inner < 16; ++j_inner) {
      T_matmul_NN[((((((((int)blockIdx.x) >> 1) * 524288) + (i_inner * 4096)) + ((((int)blockIdx.x) & 1) * 2048)) + (((int)threadIdx.x) * 16)) + j_inner)] = T_matmul_NN_local[((i_inner * 16) + j_inner)];
      T_matmul_NN[(((((((((int)blockIdx.x) >> 1) * 524288) + (i_inner * 4096)) + ((((int)blockIdx.x) & 1) * 2048)) + (((int)threadIdx.x) * 16)) + j_inner) + 1024)] = T_matmul_NN_local[(((i_inner * 16) + j_inner) + 2048)];
    }
  }
}
