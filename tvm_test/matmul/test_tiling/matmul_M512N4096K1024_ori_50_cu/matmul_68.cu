...
extern "C" __global__ void __launch_bounds__(32) mymatmul_kernel0(float* __restrict__ data, float* __restrict__ kernel, float* __restrict__ T_matmul_NN) {
  ...
  for (int i_c_outer_inner_init = 0; i_c_outer_inner_init < 16; ++i_c_outer_inner_init) {
    for (int j_c_outer_inner_init = 0; j_c_outer_inner_init < 4; ++j_c_outer_inner_init) {
      for (int j_c_inner_init = 0; j_c_inner_init < 4; ++j_c_inner_init) {
        T_matmul_NN_local[(((i_c_outer_inner_init * 16) + (j_c_outer_inner_init * 4)) + j_c_inner_init)] = 0.000000e+00f;
        T_matmul_NN_local[((((i_c_outer_inner_init * 16) + (j_c_outer_inner_init * 4)) + j_c_inner_init) + 256)] = 0.000000e+00f;
      }
    }
  }
  for (int k_outer_outer = 0; k_outer_outer < 64; ++k_outer_outer) {
    ...
    for (int ax0_ax1_fused_inner_s = 0; ax0_ax1_fused_inner_s < 64; ++ax0_ax1_fused_inner_s) {
      if (((int)threadIdx.x) < 8) {
        data_shared[((((int)threadIdx.x) * 64) + ax0_ax1_fused_inner_s)] = data[((((((((int)blockIdx.x) >> 3) * 32768) + (((int)threadIdx.x) * 4096)) + ((ax0_ax1_fused_inner_s >> 4) * 1024)) + (k_outer_outer * 16)) + (ax0_ax1_fused_inner_s & 15))];
      }
    }
    for (int ax0_ax1_fused_outer_outer = 0; ax0_ax1_fused_outer_outer < 256; ++ax0_ax1_fused_outer_outer) {
      kernel_shared[((ax0_ax1_fused_outer_outer * 32) + ((int)threadIdx.x))] = kernel[(((((k_outer_outer * 65536) + ((ax0_ax1_fused_outer_outer >> 4) * 4096)) + ((((int)blockIdx.x) & 7) * 512)) + ((ax0_ax1_fused_outer_outer & 15) * 32)) + ((int)threadIdx.x))];
    }
    __syncthreads();
  ...
  for (int i_inner = 0; i_inner < 16; ++i_inner) {
    for (int j_inner = 0; j_inner < 16; ++j_inner) {
      T_matmul_NN[(((((((((int)blockIdx.x) >> 3) * 131072) + ((((int)threadIdx.x) >> 4) * 65536)) + (i_inner * 4096)) + ((((int)blockIdx.x) & 7) * 512)) + ((((int)threadIdx.x) & 15) * 16)) + j_inner)] = T_matmul_NN_local[((i_inner * 16) + j_inner)];
      T_matmul_NN[((((((((((int)blockIdx.x) >> 3) * 131072) + ((((int)threadIdx.x) >> 4) * 65536)) + (i_inner * 4096)) + ((((int)blockIdx.x) & 7) * 512)) + ((((int)threadIdx.x) & 15) * 16)) + j_inner) + 256)] = T_matmul_NN_local[(((i_inner * 16) + j_inner) + 256)];
    }
  }
}
