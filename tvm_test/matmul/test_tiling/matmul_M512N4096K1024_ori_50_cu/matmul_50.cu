...
extern "C" __global__ void __launch_bounds__(64) mymatmul_kernel0(float* __restrict__ data, float* __restrict__ kernel, float* __restrict__ T_matmul_NN) {
  ...
  for (int k_outer_outer = 0; k_outer_outer < 128; ++k_outer_outer) {
    ...
    for (int ax0_ax1_fused_outer_outer_1 = 0; ax0_ax1_fused_outer_outer_1 < 64; ++ax0_ax1_fused_outer_outer_1) {
      kernel_shared[((ax0_ax1_fused_outer_outer_1 * 64) + ((int)threadIdx.x))] = kernel[(((((k_outer_outer * 32768) + ((ax0_ax1_fused_outer_outer_1 >> 3) * 4096)) + (((int)blockIdx.x) * 512)) + ((ax0_ax1_fused_outer_outer_1 & 7) * 64)) + ((int)threadIdx.x))];
    }
    __syncthreads();
